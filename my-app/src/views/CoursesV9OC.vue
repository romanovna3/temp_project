...
// Icon names from Figma (defined early so template never sees undefined)
const ICONS = {
...
const courseFromOCRoute = computed(() => {
  ...
  if (!key) return null
  let card = openingCourseCards.find((c) =>
    c.slug === key || String(c.id) === key || c.id === Number(key)
  )
  if (!card && key === 'sicilian-defense') {
    // Route /courses/sicilian-defense has no -white/-black suffix; resolve to the Black Sicilian Defense card.
    card = openingCourseCards.find((c) => openingSlug(c.title) === key && c.type === 'Black') ||
      openingCourseCards.find((c) => openingSlug(c.title) === key)
  }
  if (!card && key !== 'sicilian-defense') {
    const id = Number(key)
    if (Number.isInteger(id) && id >= 1 && id <= openingCourseCards.length) {
      card = openingCourseCards[id - 1]
    }
  }
  if (!card) return null
  const playSide = card.type === 'White' || card.type === 'Black' ? card.type.toLowerCase() : null
  const base = {
    id: card.id,
    title: card.title,
    instructor: card.type || 'Opening',
    lines: card.linesCount ?? 0,
    thumbnail: null,
    firstMove: card.firstMove,
    coverPieces: Array.isArray(card.coverPieces) ? card.coverPieces : [],
    playSide,
  }
  // London System for White (course 12): use screenshot content – title and author
  if (card.title === 'London System' && card.type === 'White') {
    return { ...base, title: 'London System', instructor: 'chesscomopenings', lines: 7 }
  }
  return base
})
const courses = computed(() => {
  if (openingCourseIdFromRoute.value && courseFromOCRoute.value) return [courseFromOCRoute.value]
  return isVideoV2_4OrV5.value ? coursesV24 : coursesV23
})

/** OC course page: started state from sessionStorage (reviewCount etc.) when opened from a started card. Declared early so computeds below can reference it. */
const openingStartedState = ref(null)
...
const OPENING_COURSES_RAW = [
  ...
  { name: "Queen's Gambit Accepted", color: 'White', firstMove: '1. d4', against: '1. d4, 2. c4', frequency: '5%' },
...
/** Strip " For White" or " For Black" from course title for started-header display. */
function openingCourseTitleForHeader(title) {
  if (!title || typeof title !== 'string') return title || ''
  return title.replace(/\s+For (White|Black)$/i, '').trim() || title
}
...
// Two courses per opening (white and black). One card per row; same ID order as Courses.vue opening list so navigation by id resolves to the same course.
const openingCourseCards = (() => {
  ...
  for (const row of OPENING_COURSES_RAW) {
    ...
    cards.push({
      id: id++,
      slug: openingSlug(title) + (row.color === 'Black' ? '-black' : '-white'),
      title,
      firstMove,
      description,
      linesCount: 12,
      ...
      _meta: OPENING_COURSES_RAW.filter((r) => openingNameKey(r.name) === key && r.color === row.color),
    })
  }
  return cards
})()
...
/** Nothing-to-learn on Learn tab: show 100% completion. New-course: show 0%. Started from Opening page: use saved progressPercent. */
const displayCompletionPercentRounded = computed(() => {
  if (openingCourseIdFromRoute.value && openingStartedState.value?.progressPercent != null) return Math.round(openingStartedState.value.progressPercent)
...
const displayCompletionFillWidthPx = computed(() => {
  if (openingCourseIdFromRoute.value && openingStartedState.value?.progressPercent != null) return Math.min(100, openingStartedState.value.progressPercent) * 192 / 100
  if (scenarioPreset.value === 'new-course') return 0
  if (scenarioPreset.value === 'nothing-to-learn' && courseTabsActive.value === 'content') return 192
  return completionFillWidthPx.value
})

/** True when on opening course page and course is 100% complete (show check icon next to title). */
const isOpeningCourseCompleted = computed(() =>
  !!openingCourseIdFromRoute.value && displayCompletionPercentRounded.value === 100
)
...
// Preset bar dropdown options (for custom select). Scenario options match Opening Courses V1 list (global preset bar).
...
/** Set by Opening page when user clicks Practice; course page opens with Practice tab active. */
const OPENING_COURSES_V1_OPEN_IN_PRACTICE_KEY = 'openingCoursesV1OpenInPractice'

function getOpeningStartedState() {
  try {
    if (typeof sessionStorage === 'undefined') return null
    const raw = sessionStorage.getItem(OPENING_COURSES_V1_STARTED_STATE_KEY)
    if (!raw) return null
    const data = JSON.parse(raw)
    return data?.reviewCount != null ? data : null
  } catch (_) {
    return null
  }
}
...
function getInitialOpeningV1PresetBar() {
  try {
    if (typeof sessionStorage === 'undefined') return { viewportPreset: 'default', scenarioPreset: 'returning-user' }
    const raw = sessionStorage.getItem(OPENING_COURSES_V1_PRESET_BAR_KEY)
    if (!raw) return { viewportPreset: 'default', scenarioPreset: 'returning-user' }
    ...
    return {
      ...
      scenarioPreset: s && ['new-user', 'returning-user'].includes(s) ? s : 'returning-user',
    }
  } catch (_) {
    return { viewportPreset: 'default', scenarioPreset: 'returning-user' }
  }
}
...
const openingV1ScenarioPresetOptions = [
...
// Viewport preset for responsiveness testing: 'default' | 'narrow' | 'mobile'. Synced with Opening list via sessionStorage. Initialized from storage so first paint is correct when navigating from Opening list (e.g. mobile stays mobile).
const _initialPreset = getInitialOpeningV1PresetBar()
const viewportPreset = ref(_initialPreset.viewportPreset)
...
// Scenario bar preset (same as Opening V1: New User / Returning User). Synced with sessionStorage; maps to scenarioPreset for content.
const openingV1ScenarioBarPreset = ref(_initialPreset.scenarioPreset)

// Scenario preset for footer demo: 'nothing-to-learn' | 'nothing-to-practice' | 'practice-and-learn' | 'new-course'. Driven by openingV1ScenarioBarPreset (new-user → new-course, returning-user → practice-and-learn).
const scenarioPreset = ref(_initialPreset.scenarioPreset === 'new-user' ? 'new-course' : 'practice-and-learn')

function mapOpeningV1ScenarioBarToInternal(bar) {
  return bar === 'new-user' ? 'new-course' : 'practice-and-learn'
}

function persistOpeningV1PresetBar() {
  try {
...
watch(openingV1ScenarioBarPreset, (bar) => {
  scenarioPreset.value = mapOpeningV1ScenarioBarToInternal(bar)
  persistOpeningV1PresetBar()
}, { immediate: true })
...
const scenarioEffectivePracticeCount = computed(() => {
  if (openingCourseIdFromRoute.value && openingStartedState.value?.reviewCount != null) {
    return openingStartedState.value.reviewCount
  }
...
const sectionMoves = computed(() => (openingCourseIdFromRoute.value ? sectionMovesOCResolved.value : (isVideoV2_4OrV5.value ? sectionMovesV24 : sectionMovesV23)))
...
// OC (Opening Courses) course content: single section "Main Level" with 10 variations (from design)
const courseSectionsOC = [
  { id: 'main-level', name: 'Main Level', completed: 0, total: 10, status: 'not_started', videoAvailable: true },
]

// London System for White (course 12): one chapter "London System" with 7 lines
...
const FRENCH_DEFENSE_WHITE_MAIN_LEVEL = [
  { id: '1', text: 'Winawer Variation', completed: false },
  { id: '2', text: 'Retreat Variation', completed: false },
  { id: '3', text: 'Retreat Variation with ...cxd4', completed: false },
  { id: '4', text: 'Black Trades Too Early', completed: false },
  { id: '5', text: 'Black Blocks the c-pawn', completed: false },
  { id: '6', text: 'Classical Variation', completed: false },
  { id: '7', text: 'Classical Variation with ...Ne4', completed: false },
  { id: '8', text: 'Rubinstein Variation', completed: false },
  { id: '9', text: 'Rubinstein Variation with 4...Nf6', completed: false },
...
const courseSectionsOCResolved = computed(() => {
  const course = courseFromOCRoute.value
  if (course?.title === 'London System' && course?.playSide === 'white') {
    return courseSectionsLondonSystemWhite
  }
...
const isLondonSystemWhiteCourse = computed(() => {
  const course = courseFromOCRoute.value
  return course?.title === 'London System' && course?.playSide === 'white'
})
...
const courseSections = computed(() => (openingCourseIdFromRoute.value ? courseSectionsOCResolved.value : (isVideoV2_4OrV5.value ? courseSectionsV24 : courseSectionsV23)))
...
/** When opened from a started card (returning user), show reviewCount lines in Practice tab with actual line names from Learn tab. */
const courseSectionsForPracticeDisplay = computed(() => {
...
/** Main board: Black’s perspective (rank 8 at bottom). Updated by watcher after list selection / OC route. */
...
watchEffect(() => {
  try {
    ...
    if (panelView.value === 'courses' && isOpeningCoursesOCList.value && selectedOpeningCardId.value != null) {
    ...
    if (panelView.value === 'courses') {
      if (isOpeningCoursesOCList.value && openingFilterMoves.value.length > 0) {
...
// Get piece image path from Chess.com CDN (guard for missing piece/type e.g. cover board)
const getPieceImage = (piece) => {
  const type = piece?.type
  if (!type) return ''
  return `https://www.chess.com/chess-themes/pieces/neo/300/${type}.png`
}
...
const handleSquareClick = (square) => {
  ...
  const isOpeningV1FreePlay = panelView.value === 'courses' && isOpeningCoursesOCList.value && currentQuestionIndex.value < 0
...
watch(isOpeningCoursesOCList, (isList) => {
  if (openingOCReadyTimer) {
    clearTimeout(openingOCReadyTimer)
    openingOCReadyTimer = null
  }
  if (!isList) {
    openingOCReady.value = false
    return
  }
  openingOCReady.value = false
  openingOCReadyTimer = setTimeout(() => {
    openingOCReadyTimer = null
    openingOCReady.value = true
  }, 200)
}, { immediate: true })
...
watch(
  ...
  () => {
    ...
    if (isOpeningCoursesOCList.value && selectedOpeningCardId.value != null) {
      const card = openingCourseCards.find((c) => c.id === selectedOpeningCardId.value)
      boardViewBlack.value = card?.type === 'Black'
      return
    }
...
function onOpeningContentScroll() {
  try {
    ...
    if (!el || !isOpeningCoursesOCList.value) return
...
watch(isOpeningCoursesOCList, (isV1) => {
  if (isV1) {
...
// If Error -102 on /#/courses/opening-courses-oc: apply .cursor/rules/opening-courses-v1-crash-102.mdc
...
watch(
  () => (isOpeningCoursesOCList.value ? selectedOpeningCardId.value : null),
  (id) => {
    try {
      clearOpeningAutoMove()
      clearOpeningCardPreviewTimeout()
      ...
      if (!isOpeningCoursesOCList.value) return
...
watch([courseTabsActive, courseSectionsForPracticeDisplay], () => {
...
/** OC course page: land on Learn or Practice tab when entering (Practice if user clicked Practice on Opening page); restore play side from Opening page; expand single section so lines are visible. */
watch(openingCourseIdFromRoute, (courseId) => {
  openingStartedState.value = courseId ? getOpeningStartedState() : null
  if (courseId) {
    try {
      const openInPractice = sessionStorage.getItem(OPENING_COURSES_V1_OPEN_IN_PRACTICE_KEY) === '1'
      if (openInPractice) {
        sessionStorage.removeItem(OPENING_COURSES_V1_OPEN_IN_PRACTICE_KEY)
        courseTabsActive.value = 'stats'
      } else {
...
watch(scenarioPreset, (s) => {
  if (openingCourseIdFromRoute.value) return // OC course: keep Learn tab; don't let scenario switch us to Practice
...
onMounted(() => {
  // Restore global preset bar (viewport + scenario) from Opening list so mobile stays mobile when navigating to Course page
  try {
    const raw = sessionStorage.getItem(OPENING_COURSES_V1_PRESET_BAR_KEY)
    if (raw) {
      const preset = JSON.parse(raw)
      ...
      if (preset && typeof preset.scenarioPreset === 'string' && ['new-user', 'returning-user'].includes(preset.scenarioPreset)) {
        openingV1ScenarioBarPreset.value = preset.scenarioPreset
      }
    }
  } catch (_) {
    // ignore
...
<template>
  <div class="app dark-mode courses-v9-oc" :class="[`app--viewport-${effectiveViewportPreset}`]">
    ...
    <div class="viewport-preset-bar" role="toolbar" aria-label="Viewport and version preset">
      <div v-if="SHOW_VIEWPORT_PRESET_IN_BAR" class="viewport-preset-bar__group">
        <PresetBarSelect
          ...
          :disabled="!!openingCourseIdFromRoute"
        />
      </div>
      ...
      <div class="viewport-preset-bar__group">
        <PresetBarSelect
          v-model="openingV1ScenarioBarPreset"
          :options="openingV1ScenarioPresetOptions"
          aria-label="Scenario"
          :disabled="!!openingCourseIdFromRoute"
        />
      </div>
    ...
    <div class="layout">
      ...
      <div class="review-panel">
        <div class="panel-main">
          <!-- SidebarHeader: same as Opening Courses (Courses.vue) – back | book-stack-pawn + Openings | right placeholder -->
          <header ref="panelHeaderRef" class="panel-header sidebar-header">
            <div class="sidebar-header-left">
              <button
                type="button"
                class="sidebar-header-back"
                :aria-label="panelView === 'line' ? 'Back to chapter' : 'Back to courses'"
                @click="onHeaderBackClick"
              >
                <CcIcon name="arrow-line-left" variant="glyph" :size="20" />
              </button>
            </div>
            <div class="sidebar-header-title" :class="{ 'sidebar-header-title--line': panelView === 'line' }">
              <CcIcon v-if="panelView === 'courses'" name="book-stack-pawn" variant="color" :size="24" class="sidebar-header-icon" aria-hidden="true" />
              <span class="sidebar-header-text" :class="{ 'sidebar-header-text--truncate': panelView === 'line' }">{{ panelView === 'line' ? (isVideoV2_3OrV24 ? (selectedLine?.section?.name ?? 'Chapter') : (selectedLine?.move?.text ?? 'Line')) : 'Openings' }}</span>
            </div>
        ...
        <!-- Opening Courses OC: coach fixed at top; filter + cards scroll. Deferred so layout only renders after first paint (avoids -102). -->
        <template v-if="isOpeningCoursesOCList">
          <div v-if="openingOCReady" class="opening-v1-layout">
            ...
            >
              ...
              >
                <div class="opening-search-panel">
                  <div class="opening-search-panel__row opening-search-panel__row--inputs">
                    ...
                    <CcButton variant="secondary" size="medium" :custom-img-src="baseUrl + 'icons/sliders.png'" class="opening-search-panel__filters-btn" aria-label="Filters" @click="onOpeningFiltersClick">Filters</CcButton>
                  </div>
                  <div class="opening-meta-slot" :class="{ 'opening-meta-slot--with-chips': openingFilterMoves.length || openingKeywordTags.length }">
                    ...
                    <div v-else class="opening-courses-meta-panel" data-name="Course counter and filter OC">
              ...
              <div class="opening-v1-scroll">
                <div class="opening-course-cards-list" data-name="Opening course cards OC">
            <article
          ...
          <div
            ...
            :class="{ 'courses-content--v3': isVideoV3, 'courses-content--v5': isVideoV5, 'courses-content--v6': isVideoV6OrV7, 'courses-content--v8': isVideoV8, 'courses-content--v9': isVideoV9, 'courses-content--tabs-scroll-linked': isVideoV4OrV5OrV6 }"
          ...
          <div v-if="!isOpeningCoursesOCList && isVideoV4OrV5OrV6" ref="courseTabsWrapRef" class="course-tabs-wrap course-tabs-wrap--scroll-linked course-tabs-wrap--top">
            <cc-tab-group variant="secondary" class="course-tabs-ds" role="tablist" aria-label="Course">
              ...
              <cc-tab-item id="stats" :label="courseTabStatsLabel" :badge="(openingCourseIdFromRoute && scenarioEffectivePracticeCount === 0) ? undefined : ((isVideoV8 || isVideoV9) && scenarioPreset !== 'nothing-to-practice' && scenarioPreset !== 'new-course' ? String(scenarioEffectivePracticeCount) : undefined)" :isActive="courseTabsActive === 'stats'" @click="courseTabsActive = 'stats'" />
            </cc-tab-group>
          </div>
          <div v-else-if="!isOpeningCoursesOCList" ref="courseTabsWrapRef" class="course-tabs-wrap course-tabs-wrap--top">
            <cc-tab-group variant="secondary" class="course-tabs-ds" role="tablist" aria-label="Course">
              <cc-tab-item id="content" :label="courseTabContentLabel" :isActive="courseTabsActive === 'content'" @click="courseTabsActive = 'content'" />
              <cc-tab-item id="stats" :label="courseTabStatsLabel" :badge="(openingCourseIdFromRoute && scenarioEffectivePracticeCount === 0) ? undefined : ((isVideoV8 || isVideoV9) && scenarioPreset !== 'nothing-to-practice' && scenarioPreset !== 'new-course' ? String(scenarioEffectivePracticeCount) : undefined)" :isActive="courseTabsActive === 'stats'" @click="courseTabsActive = 'stats'" />
            </cc-tab-group>
          ...
          <!-- V9: tabs under coach, above course card (scroll with content) -->
          <div
          ...
          <!-- Content tab: video + sections list (inside drawer wrap so overlay covers it) -->
          <div v-if="!isOpeningCoursesOCList" v-show="courseTabsActive === 'content'" class="course-tab-panel" :class="{ 'course-tab-panel--drawer-open': statsPanelExpanded, 'course-tab-panel--hide-flash': learnTabFirstOpenHideFlash }" role="tabpanel" aria-label="Content">
          <!-- VideoSection (V1 only) – visible when Video button clicked; under course card; sticky -->
          <section ref="videoSectionRef" v-show="showVideoSectionVisible && !isVideoV2 && !isVideoV2_2 && !isVideoV2_3OrV24 && !isVideoV3 && !openingCourseIdFromRoute" class="video-section" data-name="VideoSection">
            <div
          ...
          <!-- V7 only: New Course – Practice tab empty state (image + copy). Also show on OC course when there are no lines to review. -->
          <template v-if="isVideoV7OrV8OrV9 && courseTabsActive === 'stats' && (scenarioPreset === 'new-course' || (openingCourseIdFromRoute && !courseSectionsForPracticeDisplay.length))">
            <div class="practice-empty-state practice-empty-state--oc" data-name="Practice Empty State OC">
          ...
          <template v-else-if="isVideoV7OrV8OrV9 && courseSectionsForPracticeDisplay.length && courseTabsActive === 'stats'">
            <!-- Practice tab: same layout as Learn with --no-chapter (no chapter row, only vertical line + line cards) -->
            <div class="sections-list sections-list--practice">
              <div
                v-for="(section, sectionIndex) in courseSectionsForPracticeDisplay"
                :key="section.id"
                class="section-item"
              ...
              >
                <div class="v23-section-sticky-wrap">
                  <div
                    class="v23-section-timeline-wrap v23-section-timeline-wrap--v4 v23-section-timeline-wrap--v6 v23-section-timeline-wrap--no-chapter"
          ...
          <div v-show="courseTabsActive === 'content'" class="course-tab-panel" :class="{ 'course-tab-panel--hide-flash': learnTabFirstOpenHideFlash }" role="tabpanel" aria-label="Content">
          ...
          <div v-if="!isOpeningCoursesOCList && (courseTabsActive === 'content' || !isVideoV7OrV8OrV9)" class="sections-list">
            <div
            ...
            >
              ...
              <template v-if="isVideoV3">
                ...
                >
                  ...
                  <section v-if="showVideoSectionVisible && section.videoAvailable && !openingCourseIdFromRoute" class="video-section video-section--inline" :data-name="`VideoSection-${section.id}`">
                    <div
                      class="video-placeholder-frame"
              ...
              <template v-else>
                ...
                <div v-if="isVideoV2_3OrV24" class="v23-section-sticky-wrap">
                  ...
                  >
                    ...
                    >
                      <section
                        v-if="(isVideoV2 || isVideoV2_2 || (isVideoV2_3OrV24 && !isVideoV6OrV7)) && showVideoSectionVisible && section.videoAvailable && isSectionOpen(section.id) && !openingCourseIdFromRoute"
                    ...
                    <section
                      v-if="isVideoV6OrV7 && courseTabsActive === 'content' && showVideoSectionVisible && section.videoAvailable && isSectionOpen(section.id) && !openingCourseIdFromRoute"
...
v-if="isVideoV6OrV7"
                ...
                >
                  ...
                  <section
                    v-if="(isVideoV2 || isVideoV2_2 || isVideoV2_3OrV24) && showVideoSectionVisible && section.videoAvailable && isSectionOpen(section.id) && !openingCourseIdFromRoute"
                    class="video-section video-section--inline"
                    :data-name="`VideoSection-${section.id}`"
                  >
                    <div
        ...
        <div class="panel-footer-container" :class="{ 'panel-footer-container--no-icon-footer': !(panelView === 'courses' || panelView === 'line') || isOpeningCoursesOCList }">
          ...
          <!-- Footer layers: color picker row + CTA + icon footer in one block so content fits -->
          <div class="panel-footer-layers">
          <!-- Buttons footer: Continue / Practice – Chapter and Lines; Read mode: Back + Continue; Opening Courses OC: Start Course only -->
          ...
          <section v-if="(panelView === 'courses' || panelView === 'line') && !isOpeningCoursesOCList" class="footer-section footer-section-toolbar" data-name="IconFooter">
            <div class="footer-icon-group" data-name="V6 Icon Button Ghost Stack">
              ...
              <button v-if="false" type="button" class="footer-icon-btn" :aria-label="(isVideoV2_4OrV5 && panelView === 'line' && currentLineType === 'ready' ? readyLineVideoVisible : showVideoSectionVisible) ? 'Hide video' : 'Show video'" :disabled="panelView === 'line' && !isLineReadMode && (currentLineType === 'info' && selectedLine?.section?.id === 'intro' || isQuizLine)" @click="openVideo">
                <CcIcon :name="(isVideoV2_4OrV5 && panelView === 'line' && currentLineType === 'ready' ? readyLineVideoVisible : showVideoSectionVisible) ? 'media-camera-video-off' : 'media-camera-video-on'" variant="glyph" :size="20" class="footer-icon" />
              </button>
              <button
...
/* Placeholder while opening-courses-oc defers layout (avoids -102) */
...
/* Course page: header (title) with color chip/label under the header. */
...
/* London course page: grey [White]/[Black] chip (same as OpeningCoursesV2 piece-color-style-v2) */
...
/* First section (e.g. Introduction): no extra top padding; keep bottom */
...
.course-card-frame--with-completion .opening-course-card__content--v7-completion .course-card-completion--in-mastery-row {
  font-size: 17px;
  align-items: flex-start;
  gap: 2px;
  height: 32px;
...
.course-card-completion__complete-label {
  font-family: var(--font-family-system, system-ui, sans-serif);
  font-size: var(--text-xs, 12px);
  font-weight: 600;
  line-height: 16px;
...
.course-card-completion__bar-row {
  display: flex;
  gap: 8px;
  justify-content: flex-start;
  align-items: center;
...
/* Practice tab list: override stats panel padding so list has 0 top, 12px left */
.course-tab-panel--stats .sections-list--practice .chapter-line-cards-list-wrapper .opening-course-cards-list.chapter-line-cards-list {
  padding-top: 0;
  padding-left: 12px;
}
...
/* Wrapper for icon footer + CTA footer + color picker row so all three layers fit in one block */
.panel-footer-layers {
  display: flex;
  flex-direction: column;
  flex-shrink: 0;
...
/* When only CTA (no color picker), add 8px top margin */
.footer-buttons-container--cta-only {
  margin-top: 8px;
}
.footer-buttons-row--pick-color {
  margin-bottom: 2px;
  height: 40px;
  justify-content: flex-start;
}
.pick-your-color-wrap {
  width: auto;
}
.footer-buttons-row {
  display: flex;
