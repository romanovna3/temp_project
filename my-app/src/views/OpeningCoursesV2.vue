<script setup>
/**
 * Opening Courses V2 – design file (separate from V1 in Courses.vue).
 * Routes: /courses/opening-courses-v2, /learn/opening-courses-v2.
 * All Opening Courses design changes should be applied here; V1 remains in Courses.vue.
 */
...
const selectedOpeningShowsColorPicker = computed(() => {
  ...
  if (openingV2ScenarioPreset.value === 'new-user') return true
...
watch(isDragging, (dragging) => {
  if (dragging && isOpeningCoursesV2.value) {
...
watchEffect(() => {
  try {
    ...
    if (panelView.value === 'courses' && isOpeningCoursesV2.value && selectedOpeningCardId.value != null) {
    ...
    if (panelView.value === 'courses') {
      if (isOpeningCoursesV2.value && openingFilterMoves.value.length > 0) {
...
const isVideoV2_2 = computed(() => route.path === '/courses/v2.2' || isOpeningCoursesV2.value)
// Match decoded path; also match when hash is encoded (#%2Flearn%2Fopening-courses-v2) so -102 is avoided
const isOpeningCoursesV2 = computed(() => {
  if (!route?.path) return false
  const p = route.path
  if (p === '/courses/opening-courses-v2' || p === '/learn/opening-courses-v2') return true
  try {
    const decoded = decodeURIComponent(p)
    return decoded === '/courses/opening-courses-v2' || decoded === '/learn/opening-courses-v2'
  } catch {
    return false
  }
})
/** Defer opening-courses-v2 layout until after first paint to avoid Error -102 (renderer crash). */
const openingV2Ready = ref(false)
let openingV2ReadyTimer = null
watch(isOpeningCoursesV2, (isV1) => {
  if (openingV2ReadyTimer) {
    clearTimeout(openingV2ReadyTimer)
...
const OPENING_V2_COACH_BUBBLE_TEXT = 'Start by making a move on the board, or pick an opening to study.'
const coachBubbleChars = computed(() => OPENING_V2_COACH_BUBBLE_TEXT.split(''))
...
watch([openingV2Ready, isOpeningCoursesV2], ([ready, isV1]) => {
  if (!isV1 || !ready) {
  ...
  if (hasPlayedCoachTypewriter.value) {
    coachBubbleTypedText.value = OPENING_V2_COACH_BUBBLE_TEXT
    return
  }
  nextTick(() => {
    let index = 0
    ...
    coachTypewriterTimer = setInterval(() => {
      index += 1
      coachBubbleTypedText.value = OPENING_V2_COACH_BUBBLE_TEXT.slice(0, index)
      if (index >= OPENING_V2_COACH_BUBBLE_TEXT.length) {
...
function getInitialOpeningV1PresetBar() {
  try {
    ...
    const raw = sessionStorage.getItem(OPENING_COURSES_V2_PRESET_BAR_KEY)
    ...
    if (viewport === 'mobile') viewport = 'mobile-a' // legacy stored "mobile" → Mobile A
    return {
...
/** Mobile A or B (opening list card density, etc.). */
const isMobileViewport = computed(
  () => effectiveViewportPreset.value === 'mobile-a' || effectiveViewportPreset.value === 'mobile-b',
)
/** Mobile B: scrollable list with board in-panel; coach 128px / 13px bubble. */
const isOpeningMobileBLayout = computed(
  () => isOpeningCoursesV2.value && panelView.value === 'courses' && effectiveViewportPreset.value === 'mobile-b',
)
/** Main column board hidden; interactive board lives in opening panel (Mobile B list only). */
const isOpeningMobileBoardInPanel = computed(() => isOpeningMobileBLayout.value)
const openingV2ScenarioPreset = ref(_initialOpeningV1Preset.scenarioPreset)
...
const openingV2RubCourseList = computed(() =>
...
/** Returning user: list chrome sticky in scroll (translateY only). New user: sticky search --opening-filter-y. */
...
/** Mobile B uses plain scroll (no translateY chrome vars). */
...
function measureOpeningSearchH() {
  requestAnimationFrame(() => {
    ...
    if (mobileB && openingV2ScenarioPreset.value === 'returning-user' && el) {
      openingSearchH.value = el.offsetHeight || 92
    } else if (openingV2ScenarioPreset.value === 'returning-user' && chrome) {
...
/** When to show Recommended + All Courses sections (New User, or Returning User on All tab). */
const showRecommendedAndAllSections = computed(() =>
  openingV2ScenarioPreset.value === 'new-user' ||
  (openingV2ScenarioPreset.value === 'returning-user' && openingV2RubActiveTab.value === 'all')
)
/** Source list for sectioned view: New User = filtered; Returning All = rest list. */
const openingCoursesListForSections = computed(() => {
  if (openingV2ScenarioPreset.value === 'new-user') return openingCoursesFiltered.value
...
function setupOpeningSearchBarResizeObserver() {
  ...
  const mobileB = effectiveViewportPreset.value === 'mobile-b'
  const target = returning && !mobileB ? openingV2ListChromeRef.value : openingSearchRef.value
  if (!target) return
...
watch([isOpeningCoursesV2, openingV2Ready, openingV2ScenarioPreset, effectiveViewportPreset], ([isV1, ready]) => {
  if (!isV1 || !ready) {
    teardownOpeningSearchBarResizeObserver()
    return
  }
...
watch([isOpeningCoursesV2, openingV2Ready], ([isV1, ready]) => {
  ...
  try {
    const presetRaw = sessionStorage.getItem(OPENING_COURSES_V2_PRESET_BAR_KEY)
    if (presetRaw) {
...
// If Error -102 on /#/courses/opening-courses-v2: apply .cursor/rules/opening-courses-v2-crash-102.mdc
...
watch(
  () => (isOpeningCoursesV2.value ? selectedOpeningCardId.value : null),
  (id) => {
    try {
      clearOpeningAutoMove()
      clearOpeningCardPreviewTimeout()
      ...
      if (!isOpeningCoursesV2.value) return
...
<template>
  <div class="app dark-mode" :class="[isOpeningCoursesV2 ? `app--viewport-${effectiveViewportPreset}` : '', isOpeningCoursesV2 ? 'app--with-preset-bar' : '']">
    ...
    <div v-if="isOpeningCoursesV2" class="viewport-preset-bar" role="toolbar" aria-label="Viewport and scenario preset">
      ...
      <div class="viewport-preset-bar__group">
        <PresetBarSelect
          v-model="openingV2ScenarioPreset"
          :options="openingV2ScenarioPresetOptions"
          aria-label="Scenario"
        />
      </div>
    </div>
    <div :class="{ 'app-body': isOpeningCoursesV2 }">
    <div class="layout">
        ...
        <div class="panel-main">
          ...
          <header ref="panelHeaderRef" class="panel-header sidebar-header">
            ...
            <div class="sidebar-header-title" :class="{ 'sidebar-header-title--line': panelView === 'line' || panelView === 'opening-course' }">
              ...
              <span class="sidebar-header-text" :class="{ 'sidebar-header-text--truncate': panelView === 'line' || panelView === 'opening-course' }">{{ panelView === 'line' ? (isVideoV2_3OrV24 ? (selectedLine?.section?.name ?? 'Chapter') : (selectedLine?.move?.text ?? 'Line')) : (panelView === 'opening-course' ? (selectedOpeningCard?.title ?? 'Course') : (isOpeningCoursesV2 ? 'Openings' : 'Courses')) }}</span>
            </div>
...
<div 
          ...
          <div
            v-if="!isOpeningCoursesV2 && isVideoV4OrV5OrV6"
          ...
          <div v-if="!isOpeningCoursesV2" v-show="courseTabsActive === 'content'" class="course-tab-panel" :class="{ 'course-tab-panel--drawer-open': statsPanelExpanded }" role="tabpanel" aria-label="Content">
          ...
          <div v-if="!isOpeningCoursesV2 && (courseTabsActive === 'content' || !isVideoV7OrV8)" class="sections-list">
            <div
...
v-if="isVideoV6OrV7"
        ...
        <div class="panel-footer-container" :class="{ 'panel-footer-container--no-icon-footer': !(panelView === 'courses' || panelView === 'line' || panelView === 'opening-course') || (isOpeningCoursesV2 && panelView !== 'opening-course') }">
          ...
          <section class="footer-section footer-section-actions" data-name="ButtonsFooter">
            ...
            <!-- Opening Courses V2 (courses view): list icon + completed card → Practice (aqua); else Learn / Start Course. -->
          ...
          <section v-if="panelView === 'opening-course' || panelView === 'line' || (panelView === 'courses' && !isOpeningCoursesV2)" class="footer-section footer-section-toolbar" data-name="IconFooter">
...
.app.app--viewport-mobile-a .opening-course-card__properties, .app.app--viewport-mobile-b .opening-course-card__properties{
  display: none;
}
...
/* Mobile B only: 128px sticky coach (13px bubble), horizontal tab scroller, in-panel board, plain flow search */
...
/* Placeholder while opening-courses-v2 defers layout (avoids -102) */
