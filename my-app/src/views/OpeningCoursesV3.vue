<script setup>
/**
 * Opening Courses V3 – design file (fork of V2; V1 remains in Courses.vue).
 * Routes: /courses/opening-courses-v3, /learn/opening-courses-v3.
 * Apply V3-only design changes here; V2 stays in OpeningCoursesV2.vue.
 */
...
const selectedOpeningListForColors = computed(() =>
  openingV3ScenarioPreset.value === 'new-user' ? openingCoursesFiltered.value : openingV3RubCourseList.value
)
...
const selectedOpeningShowsColorPicker = computed(() => {
  ...
  if (openingV3ScenarioPreset.value === 'new-user') return true
...
const OPENING_COURSES_V3_PRESET_BAR_KEY = 'openingCoursesV3PresetBar'
...
const OPENING_COURSES_V3_FILTER_COLOR_KEY = 'openingCoursesV3FilterColor'

function getStoredOpeningFilterColor() {
  try {
    const raw = sessionStorage.getItem(OPENING_COURSES_V3_FILTER_COLOR_KEY)
...
function navigateOpeningListCardToCourse(card, options) {
  ...
  try {
    ...
    sessionStorage.setItem(OPENING_COURSES_V3_PRESET_BAR_KEY, JSON.stringify({
    ...
    const startedData = isReturningUserScenario(openingV3ScenarioPreset.value) ? getOpeningCardStartedData(card) : null
    if (startedData) {
...
// White: rank 8 at top, a–h left→right (a1 bottom-left dark). Black: rank 1 at top, h–a left→right (h8 bottom-left dark).
...
watchEffect(() => {
  try {
    ...
    if (panelView.value === 'courses') {
      if (isOpeningCoursesV3.value && openingFilterMoves.value.length > 0) {
...
const tryMove = async (from, to) => {
  ...
  if (currentQuestionIndex.value < 0) {
    // Opening Courses V3: only allow moves that extend the filter sequence; record move and filter courses
...
const isOpeningCoursesV3 = computed(() => {
  ...
  const p = route.path
  if (p === '/courses/opening-courses-v3' || p === '/learn/opening-courses-v3') return true
  try {
    const decoded = decodeURIComponent(p)
    return decoded === '/courses/opening-courses-v3' || decoded === '/learn/opening-courses-v3'
  } catch {
    return false
  }
})
...
const isYourOpeningsSortContext = computed(() =>
  openingV3RubActiveTab.value === 'my-openings' && isReturningUserScenario(openingV3ScenarioPreset.value)
)
...
const openingV3ListChromeRef = ref(null)
const openingV3ScrollWrapRef = ref(null)
let lastOpeningScrollTop = 0
// RUB tab strip (inside .opening-v1-list-chrome when returning user)
const openingV3RubTabsWrapRef = ref(null)

/** Scroll-linked chrome: one measured height (search-only or tabs+search combined). */
...
function applyOpeningCoursesHeaderFilters(list, sortByOverride) {
  ...
  if (sortBy === 'recent') {
    if (isReturningUserScenario(openingV3ScenarioPreset.value)) {
...
const showRecommendedAndAllSections = computed(() =>
  ...
  (openingV3ScenarioPreset.value === 'returning-user' && openingV3RubActiveTab.value === 'all')
)
/** Source list for sectioned view: New User = filtered; Returning All = rest list. */
const openingCoursesListForSections = computed(() => {
  if (openingV3ScenarioPreset.value === 'new-user') return openingCoursesFiltered.value
  if (openingV3ScenarioPreset.value === 'returning-user' && openingV3RubActiveTab.value === 'all') return openingCoursesRestList.value
  return []
})
...
const openingRecommendedEntries = computed(() => {
  if (openingV3ScenarioPreset.value === 'returning-user' && openingV3RubActiveTab.value === 'all') {
...
watch([openingV3ScenarioPreset, openingV3RubActiveTab], () => {
  if (openingV3ScenarioPreset.value === 'new-user') {
    openingSortBy.value = 'popular'
  } else if (openingV3RubActiveTab.value === 'all' && openingSortBy.value === 'recent') {
    openingSortBy.value = 'popular'
  } else if (openingV3RubActiveTab.value === 'my-openings' && openingSortBy.value === 'popular') {
    openingSortBy.value = 'recent'
  }
}, { immediate: true })

/** When switching Your Openings ↔ All: scroll to top of the page instantly (no animation). */
watch(openingV3RubActiveTab, () => {
...
watch(
  () => (isOpeningCoursesV3.value ? selectedOpeningCardId.value : null),
  (id) => {
    try {
      clearOpeningAutoMove()
      clearOpeningCardPreviewTimeout()
  ...
  <div class="app dark-mode" :class="[usesOpeningV3Shell ? `app--viewport-${effectiveViewportPreset}` : '', showOpeningV3PresetBar ? 'app--with-preset-bar' : '']">
    ...
    <div v-if="showOpeningV3PresetBar" class="viewport-preset-bar" role="toolbar" aria-label="Viewport and scenario preset">
      ...
      <div class="viewport-preset-bar__group">
        <PresetBarSelect
          v-model="openingV3ScenarioPreset"
          :options="openingV3ScenarioPresetOptions"
          aria-label="Scenario"
        />
      </div>
    </div>
    ...
    <div class="layout">
      ...
      <!-- Right: `.review-panel` card only — MT3 Restart is a sibling under `.layout`, not in this column -->
      <div class="review-panel">
        <div class="panel-main">
          ...
          <OpeningCoursePage
            v-if="panelView === 'opening-course' && isOpeningCoursesV3"
...
<div 
              ...
              <div class="opening-v1-scroll">
                <template v-if="isReturningUserScenario(openingV3ScenarioPreset)">
                  <template v-if="openingV3ScenarioPreset === 'returning-user'">
                    <div v-if="openingV3RubCourseList.length === 0" class="opening-courses-empty-state" data-name="Opening courses empty state">
            ...
            >
              <div class="opening-course-card__inner">
                <div class="opening-course-card__content-wrap">
                  ...
                  <div class="opening-course-card__content" :class="{ 'opening-course-card__content--started': isReturningUserScenario(openingV3ScenarioPreset) && isOpeningCardStarted(card) }">
                    <template v-if="isReturningUserScenario(openingV3ScenarioPreset) && isOpeningCardStarted(card)">
                ...
                <template v-else>
                    <div class="opening-course-cards-list" data-name="Opening course cards">
                      <article
                        v-for="card in openingV3RubCourseList"
                        :key="card.id"
                        class="opening-course-card"
                        :class="[
                          `opening-course-card--variant-${OPENING_COURSE_CARD_VARIANT}`,
          ...
          <div
            v-if="!isOpeningCoursesV3 && isVideoV4OrV5OrV6"
          ...
          <div v-if="!isOpeningCoursesV3" v-show="courseTabsActive === 'content'" class="course-tab-panel" :class="{ 'course-tab-panel--drawer-open': statsPanelExpanded }" role="tabpanel" aria-label="Content">
          ...
          <div v-if="!isOpeningCoursesV3 && (courseTabsActive === 'content' || !isVideoV7OrV8)" class="sections-list">
            <div
...
.app.app--viewport-mobile-a .review-panel,
.app.app--viewport-mobile-b .review-panel {
  flex: 0 0 300px;
  width: 300px;
  height: 540px;
  border-radius: 24px;
...
/* Placeholder while opening-courses-v3 defers first paint (avoids -102) */
...
/* Search inside unified list chrome: no sticky/transform (parent moves as one block). */
...
/* Column of chrome + cards + in-scroll footer; height = content so .opening-v1-scroll-wrap stays the scroller */
.opening-v1-scroll-wrap__sheet {
  display: flex;
  flex-direction: column;
  flex: 0 0 auto;
  min-width: 0;
...
/* Opening Courses V3: no courses match filters – same layout as chapter “Nothing to review” empty state */
