...
// V9-only view: all version flags fixed so only V9 layout and behavior run. No route-based branching.
const route = useRoute()
const isVideoV2_4 = ref(true)
const isVideoV5 = ref(true)
...
const isVideoV4OrV5OrV6 = ref(true)
// V9 uses V2.4-style sections/courses; not V2/V2.2/V3
const isVideoV2 = ref(false)
const isVideoV2_2 = ref(false)
const isVideoV2_3OrV24 = ref(true)
const isVideoV3 = ref(false)
...
/** Nothing-to-learn on Learn tab: show 100% completion. New-course: show 0% completion. */
const displayCompletionPercentRounded = computed(() => {
  if (scenarioPreset.value === 'new-course') return 0
  if (scenarioPreset.value === 'nothing-to-learn' && courseTabsActive.value === 'content') return 100
  return completionPercentRounded.value
})
const displayCompletionFillWidthPx = computed(() => {
  if (scenarioPreset.value === 'new-course') return 0
  if (scenarioPreset.value === 'nothing-to-learn' && courseTabsActive.value === 'content') return 192
  return completionFillWidthPx.value
})
...
// Stats tab: 4 cards – completed variations by type (Informational, Trainable, Tactics/Puzzle, Alternatives)
/** Classify a variation (move) into stats card type from its flags and text */
function getVariationType(move) {
  if (!move || typeof move !== 'object') return 'alternatives'
  const text = (move.text || '').toLowerCase()
  if (move.info === true) return 'informational'
  if (/tactics|puzzle|quiz/.test(text)) return 'tactics'
  if (move.quiz === true && /practice|review/.test(text)) return 'trainable'
  if (move.quiz === true) return 'trainable'
  return 'alternatives'
}
...
const statsCards = computed(() => statsCardsByType.value)
...
// V7/V8: stats icon click – expand Stats (Learn: stats cards; Practice: levels panel)
...
const scenarioPresetOptions = [
  ...
  { value: 'nothing-to-practice', label: 'Nothing to Practice' },
  { value: 'nothing-to-learn', label: 'Nothing to Learn' },
]

// Practice tab: cumulative “lines studied” per level – Lk = count of lines that have reached at least Lk (e.g. 8 at L6, 9+8=17 at L5)
const LEVEL_ORDER = ['L1', 'L2', 'L3', 'L4', 'L5', 'L6', 'L7', 'L8']
const practiceMasteryCountByLevel = computed(() => {
  const byLevel = { L1: 0, L2: 0, L3: 0, L4: 0, L5: 0, L6: 0, L7: 0, L8: 0 }
  const nothingToLearn = scenarioPreset.value === 'nothing-to-learn'
  const bySection = nothingToLearn ? displayMovesBySectionIdNothingToLearn.value : null
  courseSections.value.forEach((section) => {
    const moves = nothingToLearn && bySection ? (bySection[section.id] || []) : getSectionMovesForDisplay(section)
    moves.forEach((move) => {
      if (!move.completed || !move.level) return
      if (!LEVEL_ORDER.includes(move.level)) return
      byLevel[move.level]++
    })
  })
  const counts = { L1: 0, L2: 0, L3: 0, L4: 0, L5: 0, L6: 0, L7: 0, L8: 0 }
  LEVEL_ORDER.forEach((level, idx) => {
    for (let i = idx; i < LEVEL_ORDER.length; i++) counts[level] += byLevel[LEVEL_ORDER[i]]
  })
  return counts
})

const masteryLevelItems = computed(() => {
  const counts = practiceMasteryCountByLevel.value
  return [
    { level: 'L1', title: 'Rookie', counter: counts.L1, locked: counts.L1 === 0 },
    { level: 'L2', title: 'Keen Courser', counter: counts.L2, locked: counts.L2 === 0 },
    { level: 'L3', title: 'Apprentice', counter: counts.L3, locked: counts.L3 === 0 },
    { level: 'L4', title: 'Rank Riser', counter: counts.L4, locked: counts.L4 === 0 },
    { level: 'L5', title: 'Booked Up', counter: counts.L5, locked: counts.L5 === 0 },
    { level: 'L6', title: 'Expert', counter: counts.L6, locked: counts.L6 === 0 },
    { level: 'L7', title: 'Encyclopaedic', counter: counts.L7, locked: counts.L7 === 0 },
    { level: 'L8', title: 'Master', counter: counts.L8, locked: counts.L8 === 0 },
  ]
})

/** Stats drawer: new-course shows reset (all zeros / all locked). */
const displayMasteryLevelItems = computed(() => {
  if (scenarioPreset.value !== 'new-course') return masteryLevelItems.value
  return [
    { level: 'L1', title: 'Rookie', counter: 0, locked: true },
    { level: 'L2', title: 'Keen Courser', counter: 0, locked: true },
...
function toggleSection(sectionId) {
  if (isVideoV10.value) {
    const allChaptersExpanded = !v10ChaptersCollapsed.value
    ...
    v10ExpandNoTransition.value = true
    ...
    nextTick(() => {
      ...
      requestAnimationFrame(() => {
        v10ExpandNoTransition.value = false
      })
    })
    return
  }
...
/** Practice+Learn: true after the first time user switches to Learn (so we only autoscroll once). Reset when scenario changes to practice-and-learn. */
const learnTabAutoscrollDone = ref(false)
/** Practice+Learn: last scroll position for Learn tab; restored instantly when switching back. No value yet = 0. */
const learnTabSavedScrollTop = ref(0)
/** Practice+Learn: last scroll position for Practice tab; restored instantly when switching back. No value yet = 0. */
const practiceTabSavedScrollTop = ref(0)
/** Practice+Learn: hide Learn content briefly on first open so smooth scroll doesn't flash the top. */
const learnTabFirstOpenHideFlash = ref(false)
/** Previous course tab (content | stats). Used to restore Learn scroll in nothing-to-practice when switching back from Practice. */
const previousCourseTabsActive = ref('content')
const scenarioEffectivePracticeCount = computed(() => {
  if (scenarioPreset.value === 'nothing-to-practice') return 0
  if (scenarioPreset.value === 'new-course') return 0
  if (scenarioPreset.value === 'practice-and-learn') return practiceReadyCount.value
  if (scenarioPreset.value === 'nothing-to-learn') return nothingToLearnReadyForPracticeCount.value
  return 0
})
const scenarioShowOnlyPractice = computed(() => scenarioPreset.value === 'nothing-to-learn')
...
/** Practice tab ready line: show move.level (e.g. L6, L5) when set (Nothing to learn), else getReadyLineLevelChip. */
function getPracticeReadyLineLevelLabel(move) {
  if (!move) return null
...
const displayMovesBySectionIdNothingToLearn = computed(() => {
  ...
  for (const section of sections) {
    ...
    const displayMoves = moves.map((move) => {
      ...
      globalIndex += 1
      return { ...move, completed: true, level: level || undefined }
    })
    bySection[section.id] = displayMoves
  }
  return bySection
})

/** Nothing-to-learn: total count of lines ready for practice (all lines in the scenario). */
const nothingToLearnReadyForPracticeCount = computed(() => {
  if (scenarioPreset.value !== 'nothing-to-learn') return 0
  const bySection = displayMovesBySectionIdNothingToLearn.value
  return courseSections.value.reduce((sum, section) => sum + (bySection[section.id] || []).length, 0)
})

/** Nothing-to-learn: average mastery level from display moves (L6=6 … L1=1, ready=0). Drives mastery bar and badge on Practice tab. */
const nothingToLearnMasteryAverage = computed(() => {
  if (scenarioPreset.value !== 'nothing-to-learn') return null
  const bySection = displayMovesBySectionIdNothingToLearn.value
  let sum = 0
  let count = 0
  const levelToNum = { L6: 6, L5: 5, L4: 4, L3: 3, L2: 2, L1: 1 }
  for (const section of courseSections.value) {
    const moves = bySection[section.id] || []
    for (const move of moves) {
      const n = move.level ? levelToNum[move.level] : 0
      sum += n
      count += 1
    }
  }
...
/** Display mastery level (1–8) for bar: nothing-to-learn shows 4; else ref. */
const displayMasteryLevel = computed(() => {
  if (scenarioPreset.value === 'nothing-to-learn') return 4
  return masteryLevel.value
})

/** Display mastery percent for bar fill: used by stats panel segmented track. */
const displayMasteryPercent = computed(() => {
  if (scenarioPreset.value === 'nothing-to-learn') {
    const level = displayMasteryLevel.value
    return masteryTotal > 0 ? Math.min(100, (level / masteryTotal) * 100) : 0
  }
  return masteryPercent.value
})

/** Course card mastery bar fill width in px (bar is 192px, 8 segments → level 3 = 72px). */
const COURSE_CARD_MASTERY_BAR_WIDTH_PX = 192
const displayMasteryFillWidthPx = computed(() => {
  const level = displayMasteryLevel.value
  return masteryTotal > 0 ? Math.round((level / masteryTotal) * COURSE_CARD_MASTERY_BAR_WIDTH_PX) : 0
})

/** Display level badge (e.g. L4) for course card: nothing-to-learn shows L4; else ref. */
const displayExtraDataLevelBadge = computed(() => {
  if (scenarioPreset.value === 'nothing-to-learn') return 'L4'
  return extraDataLevelBadge.value
})

/** Display level name for stats panel: nothing-to-learn uses title for average level; else ref. */
const displayExtraDataLevelName = computed(() => {
  if (scenarioPreset.value === 'nothing-to-learn') {
    const badge = displayExtraDataLevelBadge.value
    const item = masteryLevelItems.value.find((m) => m.level === badge)
    return item?.title ?? badge ?? extraDataLevelName.value
  }
  return extraDataLevelName.value
})

/** New-course on Learn tab: all moves as incomplete (not started). Keyed by section id. */
const displayMovesBySectionIdNewCourse = computed(() => {
  if (scenarioPreset.value !== 'new-course') return {}
  const bySection = {}
  for (const section of courseSections.value) {
    const moves = getSectionMoves(section)
    bySection[section.id] = moves.map((move) => ({ ...move, completed: false }))
  }
  return bySection
})

/** Same as getSectionMoves (used for display and progress so one source of truth). Nothing-to-learn on Learn tab: return all moves as completed with level. New-course on Learn tab: return all moves as incomplete. */
...
/** V10: auto-select first chapter so one header has sticky class and sticks when scrolling (visible without clicking). */
watch(
  () => (isVideoV10.value && courseSections.value?.length ? courseSections.value[0]?.id : null),
  (firstId) => {
    if (firstId != null && v10SelectedChapterId.value == null) {
      v10SelectedChapterId.value = firstId
      v10ChaptersCollapsed.value = false
    }
  },
  { immediate: true }
...
function getPracticeInLabelForMove(move) {
  ...
  if (level === 'L2') return '1 week'
  return '3 days'
}

/** Nothing-to-practice: closest practice available time (placeholder). */
const scenarioNothingToPracticeClosestPracticeIn = computed(() => '3 hours')
...
/** V8 only: line cover icon. INFO → circle-fill-info (no chip). Others → deterministic sequence with runs (same icon repeated) and more Trainable (~35%, similar to INFO). */
const LINE_COVER_ICONS_V8 = ['document-book-open', 'game-type-puzzle', 'arrow-fork-vertical', 'board-simple-pawn']
/** Weights for non-INFO: Reference 21%, Puzzle 22%, Alternative 21%, Trainable 36% */
const LINE_COVER_WEIGHTS_V8 = [21, 22, 21, 36] // sum 100
/** Alternative icon must not be used for lines that appear in Practice (ready/completed). */
const LINE_COVER_ICON_INDEX_ALTERNATIVE = 2
const LINE_COVER_ICON_INDEX_TRAINABLE = 3
...
const lineCoverIconByKeyV8 = computed(() => {
  ...
  sections.forEach((section) => {
    ...
    moves.forEach((move, i) => {
      const key = `${section.id}-${move.id}`
      if (move.info) {
        out[key] = 'circle-fill-info'
        prevIcon = null
        return
      }
      // The Board: Files – always trainable icon (line is practiceable).
      if (section.id === 'learning-the-game' && move.id === '2') {
        out[key] = 'board-simple-pawn'
        prevIcon = 'board-simple-pawn'
        return
      }
      const seed = simpleHash(`${section.id}-${i}-${move.id}`)
      const reuseRoll = seed % 100
      if (prevIcon !== null && reuseRoll < REUSE_PERCENT) {
        out[key] = prevIcon
      } else {
        const pickRoll = simpleHash(`${key}-v8`) % 100
        let acc = 0
        let chosen = 0
        for (let w = 0; w < LINE_COVER_WEIGHTS_V8.length; w++) {
          acc += LINE_COVER_WEIGHTS_V8[w]
          if (pickRoll < acc) {
            chosen = w
            break
          }
        }
        // Alternative lines cannot be trained; never use ALT icon for lines that appear in Practice.
        const inPractice = move.completed === true
        if (inPractice && chosen === LINE_COVER_ICON_INDEX_ALTERNATIVE) {
          chosen = LINE_COVER_ICON_INDEX_TRAINABLE
        }
        prevIcon = LINE_COVER_ICONS_V8[chosen]
        out[key] = prevIcon
      }
    })
  })
...
function getLineCoverIconV8(section, move) {
  if (move?.info) return 'circle-fill-info'
  // The Board: Files – always trainable icon.
  if (section?.id === 'learning-the-game' && move?.id === '2') return 'board-simple-pawn'
  const key = `${section?.id ?? ''}-${move?.id ?? ''}`
  return lineCoverIconByKeyV8.value[key] ?? LINE_COVER_ICONS_V8[0]
}
...
// V9: isVideoV2_3OrV24 defined at top as ref
...
// V9: isVideoV3 defined at top as ref
...
// V10: reactive visible height of tabs (0 when hidden) so stack height has one source of truth
const tabsVisibleHeightPx = ref(48)

// V9 only: synthetic stack height (V10 uses layout-driven stack, no inline height)
const v9StackHeightStyle = computed(() => {
  if (!isVideoV9.value) return undefined
  return { height: 'calc(var(--header-h, 0px) + var(--tabs-visible, 48px) + var(--course-h, 120px))' }
})

// V4/V5: Scroll-linked tabs. V10: single source of truth for top stack (no gap above sticky chapter).
const COURSE_HEADER_H_PX = 0
const courseTabsH = ref(48)
/** Temporary debug: set to true to log V10 stack/sticky values (remove after fixing gap) */
const DEBUG_V10_STACK = false
/** Temporary debug: set to true to compare Learn vs Practice during upward scroll (tabs visible height, sticky top, transform, CSS vars, tab mode) */
const DEBUG_V10_SCROLL_UP = false
function debugV10Stack(label, varsTarget) {
  if (!DEBUG_V10_STACK || !isVideoV10.value || !varsTarget) return
  const scrollEl = v9ScrollRef.value
  const scrollPaddingTop = scrollEl ? getComputedStyle(scrollEl).paddingTop || '0' : '0'
  const topStackPx = COURSE_HEADER_H_PX + tabsVisibleHeightPx.value + (courseCardHeightPx.value || 120)
  console.log('[V10 stack]', label, {
    headerH: COURSE_HEADER_H_PX,
    tabsVisibleHeightPx: tabsVisibleHeightPx.value,
    courseCardHeightPx: courseCardHeightPx.value,
    topStackPx,
    scrollPaddingTop,
    tabsY
  })
}
/** Debug: scroll container + stack breakdown – find exact source of extra ~56px (call from console when on V10) */
function measureV10StickyGap() {
  if (!isVideoV10.value || !courseCardV9Ref.value || !v9ScrollRef.value) {
    console.warn('[V10 gap] Need V10, courseCardV9Ref, v9ScrollRef')
    return
  }
  const courseCardEl = courseCardV9Ref.value
  const scrollContainerEl = v9ScrollRef.value
  const firstSticky = scrollContainerEl.querySelector('.chapter-v2--sticky-title-v23, .chapter-v2--v10-selected')
  if (!firstSticky) {
    console.warn('[V10 gap] No sticky chapter found')
    return
  }
  const courseCardBottom = courseCardEl.getBoundingClientRect().bottom
  const scrollContainerTop = scrollContainerEl.getBoundingClientRect().top
  const stickyChapterTop = firstSticky.getBoundingClientRect().top
  const gapToCard = stickyChapterTop - courseCardBottom
  const gapToScrollTop = stickyChapterTop - scrollContainerTop
  const parent = scrollContainerEl.parentElement
  const parentStyle = parent ? getComputedStyle(parent) : null
  const stackEl = document.querySelector('.courses-content--v9-stack')
  const stackRect = stackEl ? stackEl.getBoundingClientRect() : null
  const stackHeightPx = stackEl ? stackEl.offsetHeight : null
  const headerHeight = COURSE_HEADER_H_PX
  const tabsVisibleHeight = tabsVisibleHeightPx.value
  const courseCardHeight = courseCardHeightPx.value ?? 120
  const expectedStackHeight = headerHeight + tabsVisibleHeight + courseCardHeight
  const panelTop = parent ? parent.getBoundingClientRect().top : null
  const expectedScrollContainerTop = panelTop != null ? panelTop + expectedStackHeight : null
  const scrollContainerOffsetFromExpected = expectedScrollContainerTop != null ? Math.round((scrollContainerTop - expectedScrollContainerTop) * 10) / 10 : null

  const stackChildrenBreakdown = []
  if (stackEl) {
    const directChildren = Array.from(stackEl.children)
    directChildren.forEach((child, i) => {
      const cs = getComputedStyle(child)
      const rect = child.getBoundingClientRect()
      stackChildrenBreakdown.push({
        index: i,
        className: child.className || child.tagName,
        offsetHeight: child.offsetHeight,
        getBoundingClientRectHeight: Math.round(rect.height * 10) / 10,
  ...
  const stackComputedHeight = stackEl ? getComputedStyle(stackEl).height : null
  const stackInlineHeight = stackEl ? stackEl.style.height : null

  console.log('[V10 stack breakdown]', {
    headerHeight,
    tabsVisibleHeight,
    courseCardHeight,
    expectedVisualStackHeight: expectedStackHeight,
    stackHeightPx,
    stackInlineHeight,
    stackComputedHeight,
    stackHeightMismatch: stackHeightPx != null ? Math.round((stackHeightPx - expectedStackHeight) * 10) / 10 : null,
    stackDirectChildrenHeights: stackChildrenBreakdown,
    scrollContainerTop: Math.round(scrollContainerTop * 10) / 10,
    expectedScrollContainerTop: expectedScrollContainerTop != null ? Math.round(expectedScrollContainerTop * 10) / 10 : null,
    scrollContainerOffsetFromExpected,
    courseCardBottom: Math.round(courseCardBottom * 10) / 10,
    gapToCard: Math.round(gapToCard * 10) / 10,
    gapToScrollTop: Math.round(gapToScrollTop * 10) / 10
  })

  const stickyCs = getComputedStyle(firstSticky)
  const stickyRect = firstSticky.getBoundingClientRect()
  const parentChain = []
  let node = firstSticky.parentElement
  let depth = 0
  while (node && node !== scrollContainerEl && depth < 8) {
  ...
  console.log('[V10 sticky chapter]', {
    computedTop: stickyCs.top,
    computedTransform: stickyCs.transform,
    computedTranslate: stickyCs.translate,
    marginTop: stickyCs.marginTop,
    paddingTop: stickyCs.paddingTop,
    minHeight: stickyCs.minHeight,
    position: stickyCs.position,
    getBoundingClientRectTop: Math.round(stickyRect.top * 10) / 10,
    scrollContainerTop: Math.round(scrollContainerTop * 10) / 10,
    parentChain
  })
}
...
function onCoursesContentScroll() {
  ...
  tabsScrollRafId = requestAnimationFrame(() => {
    ...
    const st = typeof scrollEl.scrollTop === 'number' ? scrollEl.scrollTop : 0
    ...
    if (st < tabsStickyStartScrollTop) {
      ...
      if (isVideoV10.value) tabsVisibleHeightPx.value = H
    ...
    const tabsVisiblePx = Math.max(0, tabsY + H)
    varsTarget.style.setProperty('--tabs-y', tabsY + 'px')
    varsTarget.style.setProperty('--tabs-visible', tabsVisiblePx + 'px')
    if (isVideoV10.value) tabsVisibleHeightPx.value = tabsVisiblePx
    if (DEBUG_V10_SCROLL_UP && isVideoV10.value && scrollEl) {
      const firstSticky = scrollEl.querySelector('.chapter-v2--sticky-title-v23, .chapter-v2--v10-selected')
      const stickyCs = firstSticky ? getComputedStyle(firstSticky) : null
      const stickyTop = firstSticky ? firstSticky.getBoundingClientRect().top : null
      const scrollTop = scrollEl.getBoundingClientRect().top
      console.log('[V10 scroll]', {
        mode: courseTabsActive.value === 'content' ? 'Learn' : 'Practice',
        tabsVisibleHeightPx: tabsVisiblePx,
        tabsY,
        stickyChapterTop: stickyTop != null ? Math.round(stickyTop * 10) / 10 : null,
        scrollContainerTop: Math.round(scrollTop * 10) / 10,
        stickyComputedTop: stickyCs?.top ?? null,
        stickyTransform: stickyCs?.transform ?? null,
        varsTabsVisible: varsTarget.style.getPropertyValue('--tabs-visible'),
        varsTabsY: varsTarget.style.getPropertyValue('--tabs-y'),
...
function setupCourseTabsScrollListener() {
  ...
  if (isVideoV4OrV5OrV6.value) {
    ...
    target.addEventListener('scroll', onCoursesContentScroll, { passive: true })
    ...
    el.style.setProperty('--tabs-h', courseTabsH.value + 'px')
    el.style.setProperty('--tabs-y', tabsY + 'px')
    const tabsVisiblePx = tabsY + courseTabsH.value
    el.style.setProperty('--tabs-visible', tabsVisiblePx + 'px')
    ...
    if (isVideoV10.value) {
      tabsVisibleHeightPx.value = tabsVisiblePx
      debugV10Stack('setup', el)
      if (DEBUG_V10_STACK) nextTick(() => requestAnimationFrame(measureV10StickyGap))
    }
    computeTabsStickyStart()
    if (courseCardV9Ref.value) {
      courseCardResizeObserver = new ResizeObserver(() => {
...
/* When viewport is narrow (300px) or mobile, scale video proportionally (300/460 ≈ 0.652) */
...
const VIDEO_FORMATS = Object.freeze({
  large: { height: 250, width: null }, // width from container aspect
  small: { height: 112, width: 198 },
  none: { height: 0, width: 0 }
})
const videoFormat = ref('large')

const VIDEO_LARGE_H = 250
...
const videoFrameHeight = computed(() => {
  if (isResizingVideo.value && videoDragPreviewHeight.value != null) {
    return Math.round(videoDragPreviewHeight.value * videoViewportScale.value)
  }
  const base = VIDEO_FORMATS[videoFormat.value].height
  return Math.round(base * videoViewportScale.value)
})
const videoFrameWidth = computed(() => {
  const h = videoFrameHeight.value
  const scale = videoViewportScale.value
  const rawW = videoSectionRef.value?.clientWidth
  const defaultW = scale < 1 ? VIDEO_PANEL_WIDTH_NARROW : VIDEO_PANEL_WIDTH_DEFAULT
  const w = (rawW && rawW > 0) ? rawW : defaultW
  if (isResizingVideo.value && videoDragPreviewHeight.value != null) {
    const baseH = videoDragPreviewHeight.value
    if (baseH <= 0) return 0
    if (baseH < VIDEO_SMALL_H) return Math.round(198 * (baseH / VIDEO_SMALL_H) * scale)
    if (baseH < VIDEO_LARGE_H) return Math.round((198 + (VIDEO_PANEL_WIDTH_DEFAULT - 198) * ((baseH - VIDEO_SMALL_H) / (VIDEO_LARGE_H - VIDEO_SMALL_H))) * scale)
    return Math.round(VIDEO_PANEL_WIDTH_DEFAULT * scale)
  }
  const fmt = VIDEO_FORMATS[videoFormat.value]
  if (fmt.width !== null) return Math.round(fmt.width * scale)
  return w
})
...
const lineViewVideoFrameHeight = computed(() => {
  if (lineViewIsResizingVideo.value && lineViewVideoDragPreviewHeight.value != null) {
    return Math.round(lineViewVideoDragPreviewHeight.value * videoViewportScale.value)
  }
  const base = VIDEO_FORMATS[lineViewVideoFormat.value].height
  return Math.round(base * videoViewportScale.value)
})
const lineViewVideoFrameWidth = computed(() => {
  ...
  const defaultW = scale < 1 ? VIDEO_PANEL_WIDTH_NARROW : VIDEO_PANEL_WIDTH_DEFAULT
  const w = (rawW > 0) ? rawW : ((scrollBodyW > 0) ? scrollBodyW : defaultW)
  if (lineViewIsResizingVideo.value && lineViewVideoDragPreviewHeight.value != null) {
    const h = lineViewVideoDragPreviewHeight.value
    if (h <= 0) return 0
    if (h < VIDEO_SMALL_H) return Math.round(198 * (h / VIDEO_SMALL_H) * scale)
    if (h < VIDEO_LARGE_H) return Math.round((198 + (w - 198) * ((h - VIDEO_SMALL_H) / (VIDEO_LARGE_H - VIDEO_SMALL_H))) * scale)
...
// V2 / V2.2 / V2.3 / V6 / V7 / V8: show video by default (per-chapter blocks on Learn tab)
watch([isVideoV2, isVideoV2_2, isVideoV2_3OrV24, isVideoV3, isVideoV6OrV7], ([v2, v22, v23, v3, v67]) => {
  if (v2 || v22 || v23 || v3 || v67) showVideoSection.value = true
}, { immediate: true })
...
/** Delay (ms) to keep Learn content hidden on first open so smooth scroll doesn't flash the top. */
const LEARN_FIRST_OPEN_HIDE_MS = 120

/** Restore scroll position with a hard snap (no animation). Temporarily forces scroll-behavior: auto so CSS smooth cannot animate. */
function snapScrollTop(el, top) {
...
/** When switching tabs: close stats panel. Save scroll when leaving a tab. Practice+Learn: first switch to Learn = autoscroll; later = snap restore. No saved position = 0. Other scenarios: scroll to top (snap). After restore, keep tabs visible (no scroll-linked hide). */
  ...
  if (active === 'stats' || active === 'content') {
    nextTick(() => {
      ...
      if (isFirstOpenToLearn) {
        ...
        }, LEARN_FIRST_OPEN_HIDE_MS)
      } else if (active === 'content' && scenarioPreset.value === 'practice-and-learn') {
        snapScrollTop(el, learnTabSavedScrollTop.value)
        keepTabsVisibleAfterTabSwitch()
      } else if (active === 'stats' && scenarioPreset.value === 'practice-and-learn') {
        snapScrollTop(el, practiceTabSavedScrollTop.value)
        keepTabsVisibleAfterTabSwitch()
      } else if (active === 'content' && scenarioPreset.value === 'nothing-to-practice' && previousCourseTabsActive.value === 'stats') {
        snapScrollTop(el, learnTabSavedScrollTop.value)
        keepTabsVisibleAfterTabSwitch()
      } else if (active === 'content' && scenarioPreset.value === 'nothing-to-practice') {
        keepTabsVisibleAfterTabSwitch()
      } else {
        snapScrollTop(el, 0)
        keepTabsVisibleAfterTabSwitch()
      }
      previousCourseTabsActive.value = active
    })
  }
})

/** Sync tab and autoscroll flag when scenario changes: Practice+Learn → land on Practice; Nothing to practice / New course → land on Learn + autoscroll on first load. */
watch(scenarioPreset, (s) => {
  if (s === 'practice-and-learn') {
    courseTabsActive.value = 'stats'
    learnTabAutoscrollDone.value = false
    learnTabFirstOpenHideFlash.value = false
  } else if (s === 'nothing-to-practice' || s === 'new-course') {
...
onMounted(() => {
  if (typeof window !== 'undefined' && isVideoV10.value) {
    (window).__measureV10StickyGap = measureV10StickyGap
  }
      ...
      <div class="review-panel">
        ...
        <template v-if="isOpeningCoursesV1">
          ...
          <div
            ...
            :class="{ 'courses-content--v3': isVideoV3, 'courses-content--v5': isVideoV5, 'courses-content--v6': isVideoV6OrV7, 'courses-content--v8': isVideoV8, 'courses-content--v9': isVideoV9, 'courses-content--v10': isVideoV10, 'courses-content--tabs-scroll-linked': isVideoV4OrV5OrV6 }"
          ...
          <template v-if="isVideoV9 || isVideoV10">
            <!-- V10: layout-driven stack – tabs in clipped container, course card in flow; no absolute, no synthetic height. -->
            <template v-if="isVideoV10">
              <div
                ref="coursesContentV9WrapperRef"
                class="courses-content--v9-stack courses-content--v9-stack--v10-layout"
              >
                <div class="courses-content--v9-tabs-visible-wrap" :style="{ height: tabsVisibleHeightPx + 'px', overflow: 'hidden' }">
                  <div
                        ...
                        <div class="opening-course-card__content opening-course-card__content--v7-completion" :class="{ 'opening-course-card__content--v7-practice': courseTabsActive === 'stats' }">
                          ...
                          <div v-if="courseTabsActive === 'content'" class="course-card-mastery-row" data-name="Progress">
                            <div class="course-card-mastery-group">
                              <div class="course-card-completion course-card-completion--in-mastery-row" data-name="Completion" role="progressbar" :aria-valuenow="displayCompletionPercentRounded" aria-valuemin="0" aria-valuemax="100" aria-label="Course completion">
                                ...
                                <p class="course-card-completion__label" :class="{ 'course-card-completion__label--compact': isNarrowOrMobileViewport }">{{ isNarrowOrMobileViewport ? 'Complete: ' + displayCompletionPercentRounded + '%' : displayCompletionPercentRounded + '%' }}</p>
                              </div>
                            </div>
                          ...
                          <div v-else-if="courseTabsActive === 'stats'" class="course-card-mastery-row" data-name="Mastery progress">
                            <div class="course-card-mastery-group">
                              ...
                              <template v-else-if="scenarioPreset === 'new-course'">
                                <!-- New course: empty mastery bar (hidden on smaller screens) -->
                                <div v-if="!isNarrowOrMobileViewport" class="course-card-mastery-bar" role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="8" aria-label="Mastery level">
            ...
            <!-- V9: synthetic stack height + absolute tabs/card. -->
            <template v-else>
              <div
                ref="coursesContentV9WrapperRef"
          ...
          <template v-else>
          <!-- Non-V9: tabs and course card in scroll flow -->
          ...
          <template v-if="isVideoV10">
            <div ref="v9ScrollRef" class="courses-content courses-content--v10 courses-content--v9-scroll courses-content--line-items-no-click" :class="{ 'v10-chapters-collapsed': v10ChaptersCollapsed, 'v10-expand-no-transition': v10ExpandNoTransition }" @scroll.passive="onCoursesContentScroll">
            ...
            <div v-if="(isVideoV7OrV8OrV9 || isVideoV10) && statsPanelExpanded" class="stats-drawer" :class="{ 'stats-drawer--fill-height': isNarrowOrMobileViewport }" role="dialog" aria-label="Stats" :style="statsDrawerFixedStyle">
              <div class="stats-drawer__inner">
                ...
                <div class="course-progress-header stats-drawer__title stats-drawer__title--hidden" data-name="Stats Header" aria-hidden="true">
                  <span class="course-progress-title">{{ courseTabsActive === 'stats' ? 'Mastery stats' : 'Stats' }}</span>
                </div>
              <template v-if="courseTabsActive === 'content'">
                <StatsCards :cards="displayStatsCards" />
              </template>
              <template v-else-if="courseTabsActive === 'stats'">
          ...
          <!-- Content tab: video + sections list. V10: do not render this panel so Learn uses only the sections-list panel below – same single-panel structure as Practice, no gap on scroll up. -->
          <div v-if="!isOpeningCoursesV1 && !isVideoV10" v-show="courseTabsActive === 'content'" class="course-tab-panel" :class="{ 'course-tab-panel--drawer-open': statsPanelExpanded, 'course-tab-panel--hide-flash': learnTabFirstOpenHideFlash }" role="tabpanel" aria-label="Content">
          ...
          <!-- Stats tab: Progress + Mastery. V7 Practice: mastery is on course card; sections list same design as Learn. V10: same panel used for both tabs so Learn has identical structure to Practice (no gap on scroll up). -->
          <div v-show="courseTabsActive === 'stats' || isVideoV10" class="course-tab-panel course-tab-panel--stats" :class="{ 'course-tab-panel--v7': (isVideoV7OrV8OrV9 || isVideoV10), 'course-tab-panel--drawer-open': statsPanelExpanded }" role="tabpanel" aria-label="Stats">
          <!-- V10 Learn: same panel as Practice, sections list first so sticky behavior is identical -->
          <template v-if="isVideoV10 && courseTabsActive === 'content'">
              ...
              >
                <div class="v23-section-sticky-wrap">
                  <div
                    ...
                    :ref="(el) => setSectionLineWrapRef(section.id, el)"
                  >
                    <div class="v23-section-timeline-wrap__line" aria-hidden="true" />
                    <component
                      :is="'button'"
                      type="button"
                      class="chapter-v2 chapter-v2--header chapter-v2--sticky-title-v23 chapter-v2--v4-timeline chapter-v2--v6-timeline-right chapter-v2--no-accordion"
                      :class="{ 'chapter-v2--v10-selected': v10SelectedChapterId === section.id }"
                    ...
                    >
                      ...
                      >
                        <section
                          v-if="(isVideoV2 || isVideoV2_2 || (isVideoV2_3OrV24 && !isVideoV6OrV7)) && showVideoSectionVisible && section.videoAvailable && isSectionOpen(section.id)"
                          class="video-section video-section--inline"
                          :data-name="`VideoSection-${section.id}`"
                        >
                          <div class="video-placeholder-frame" :style="{ width: videoFrameWidth + 'px', height: videoFrameHeight + 'px' }">
                            <button v-if="videoFormat !== 'none'" type="button" class="video-play-button" :aria-label="(isVideoV2_2 && v22PlayingSectionId === section.id) || (isVideoV2_3OrV24 && v23PlayingSectionId === section.id) ? 'Pause video' : 'Play video'" @click="isVideoV2_2 ? toggleV22VideoPlay(section.id) : isVideoV2_3OrV24 ? toggleV23VideoPlay(section.id) : undefined">
                              <CcIcon :name="(isVideoV2_2 && v22PlayingSectionId === section.id) || (isVideoV2_3OrV24 && v23PlayingSectionId === section.id) ? 'media-control-pause' : 'media-control-play'" variant="glyph" :size="24" class="video-play-icon" />
                            </button>
                          </div>
                        </section>
                        <section
                          v-if="isVideoV6OrV7 && courseTabsActive === 'content' && showVideoSectionVisible && section.videoAvailable && isSectionOpen(section.id)"
                          class="video-section video-section--inline video-section--scrollable"
                          :data-name="`VideoSection-${section.id}`"
                        >
                          <div class="video-placeholder-frame" :style="{ width: videoFrameWidth + 'px', height: videoFrameHeight + 'px', backgroundColor: '#000000' }">
                            <button v-if="videoFormat !== 'none'" type="button" class="video-play-button" aria-label="Play video">
                              <CcIcon name="media-control-play" variant="glyph" :size="24" class="video-play-icon" />
                            </button>
                          </div>
                        </section>
                      ...
                      <Transition name="chapter-moves">
                        <div v-if="isSectionOpen(section.id) && getSectionMovesForDisplay(section).length" class="move-list-wrap">
                          <div v-if="isVideoV2_4OrV5" class="chapter-line-cards-list-wrapper">
                            <div class="opening-course-cards-list chapter-line-cards-list" role="list" data-name="Lines" aria-label="Lines">
                              <article
                              ...
                              >
                                <div class="chapter-line-card__body chapter-line-card__body--no-click" :title="move.text">
                                  <div class="opening-course-card__inner">
                                    <div class="opening-course-card__content-wrap">
                                      <div class="opening-course-card__cover-wrap">
                                        <div
                                          ...
                                          :class="{ 'chapter-line-card__intro-cover--v6-incomplete': !move.completed, 'chapter-line-card__intro-cover--v6-completed': move.completed }"
                                          aria-hidden="true"
                                        >
                                          <CcIcon :name="(isVideoV8 || isVideoV9 || isVideoV10) ? getLineCoverIconV8(section, move) : (move.info ? 'document-book-open' : 'game-type-puzzle')" variant="glyph" :size="20" class="chapter-line-card__intro-cover-icon chapter-line-card__intro-cover-icon--v6" />
                                        </div>
                                        <div
                                        ...
                                        >
                                          ...
                                          <div class="chapter-line-card__intro-cover-light">
                                            <div class="chapter-line-card__intro-cover-light-inner">
                                              <img v-if="move.quiz" :src="baseUrl + 'icons/misc/quiz-dice.png'" alt="" class="chapter-line-card__intro-cover-icon chapter-line-card__intro-cover-icon--img" width="32" height="32" aria-hidden="true" />
                                              <CcIcon v-else-if="isVideoV5OrV6" :name="lineCoverByKey[`${section.id}-${move.id}`]?.pieceIcon || 'piece-white-pawn'" variant="color" :customSize="28" class="chapter-line-card__intro-cover-icon" />
                                              <CcIcon v-else :name="lineCoverByKey[`${section.id}-${move.id}`]?.pieceIcon || 'piece-white-rook'" variant="color" :customSize="32" class="chapter-line-card__intro-cover-icon" />
                                            </div>
                                          </div>
                                        </div>
                                        <div v-else class="course-cover-board chapter-line-card__board" aria-hidden="true">
                                          <svg class="course-cover-board__svg" fill="none" preserveAspectRatio="none" viewBox="0 0 96 96" aria-hidden="true">
                                            <g id="course-cover-board-grid">
                                              <path d="M48 24H24V48H48V24Z" fill="var(--color-chess-light, #EBECD0)" />
                                              <path d="M96 24H72V48H96V24Z" fill="var(--color-chess-light, #EBECD0)" />
                                      ...
                                      <div class="opening-course-card__content">
                                        <div class="chapter-line-card__header-row">
                                          <div class="chapter-line-card__title-wrap">
                                            ...
                                            <CcChip v-if="move.info && !(isVideoV8 || isVideoV9 || isVideoV10)" label="INFO" color="gray" variant="translucent" :is-uppercase="false" label-class="move-item-info-chip-label chapter-line-card__info-chip-inline" />
                                          </div>
                                          <div class="chapter-line-card__header-indicators">
                                            <CcChip v-if="move.completed && move.level && !isVideoV6OrV7" :label="move.level" color="aqua" variant="translucent" :is-uppercase="false" label-class="move-item-level-chip-label" />
                                          </div>
                                        </div>
                                ...
                                <div class="chapter-line-card__timeline-col" aria-hidden="true" :ref="lineIndex === getSectionMovesForDisplay(section).length - 1 ? (el => setSectionLastCardColRef(section.id, el)) : undefined">
                                  <span
                          ...
                          <div v-else class="move-list" role="list" data-name="Lines" aria-label="Lines">
                            <div
                              ...
                              :key="`${section.id}-${move.id}`"
                              class="move-item"
                              :class="[ { 'move-item--inactive': !move.completed }, `move-item--${getLineType(move)}` ]"
                              role="listitem"
                              data-name="Line"
                              :data-move-id="`${section.id}-${move.id}`"
                              @mouseenter="setHoveredChapterLine(section, move)"
                              ...
                              <div class="move-item-content">
                                <div class="move-item-inner">
                                  ...
                                  <span v-else-if="move.completed && move.level" class="move-item-level-wrap" aria-hidden="true">
                                    <CcChip :label="move.level" color="aqua" variant="translucent" :is-uppercase="false" label-class="move-item-level-chip-label" />
                                  </span>
          ...
          <!-- Practice tab content (and non-V10 Stats): Progress, Mastery, empty state, Practice sections list -->
          <template v-else-if="courseTabsActive === 'stats'">
          ...
          <!-- Stats cards: only in drawer on Learn (removed from inline) -->
          ...
          <div v-if="!(isVideoV7OrV8OrV9 || isVideoV10)" class="course-progress course-progress-mastery" data-name="Mastery Container">
            <div class="course-progress-header" data-name="Header">
              ...
              <div class="course-progress-mastery-next-level extra-data-next-level" data-name="NextLevel">
                ...
                <div class="extra-data-level-chip" data-name="LevelChip">
                  <CcChip
                    :label="displayExtraDataLevelBadge"
                    color="aqua"
                    variant="translucent"
                    :is-uppercase="false"
                    label-class="extra-data-level-chip-label"
                  />
                  <span class="extra-data-level-name">{{ displayExtraDataLevelName }}</span>
                </div>
              </div>
            </div>
            <div class="course-progress-segmented-track" role="progressbar" :aria-valuenow="displayMasteryLevel" :aria-valuemin="0" :aria-valuemax="masteryTotal" :aria-label="'Mastery: Level ' + displayMasteryLevel + ' of ' + masteryTotal">
              <div class="course-progress-segmented-segments">
                <div v-for="i in 8" :key="i" class="course-progress-segmented-segment" />
              </div>
              <div class="course-progress-segmented-fill" :style="{ width: displayMasteryPercent + '%' }">
          ...
          <!-- Content tab (non-V10 only): V10 Learn uses same panel as Practice above -->
          <div v-if="!isVideoV10" v-show="courseTabsActive === 'content'" class="course-tab-panel" :class="{ 'course-tab-panel--hide-flash': learnTabFirstOpenHideFlash }" role="tabpanel" aria-label="Content">
          ...
          <div v-if="!isOpeningCoursesV1 && (courseTabsActive === 'content' || !(isVideoV7OrV8OrV9 || isVideoV10))" class="sections-list">
            <div
              ...
              class="section-item"
              :data-section-id="section.id"
              :ref="(isVideoV2_3OrV24 || isVideoV10) ? (el) => setV23SectionItemRef(section.id, el) : undefined"
            >
              ...
              <template v-else>
                <!-- V2.3/V10: chapter as button (accordion) or div (header only); sticky wrap for stick-at-top. V10: match Practice (same timeline/chapter classes so sticky fix applies). -->
                <div v-if="isVideoV2_3OrV24 || isVideoV10" class="v23-section-sticky-wrap">
                  <!-- V2.3/V4: wrapper for layout; V4 draws one vertical line from chapter checkmark down through cards (masked at last card). V10: always --v4 and --v6 like Practice. -->
                  <div
                    class="v23-section-timeline-wrap"
                    :class="{ 'v23-section-timeline-wrap--v4': isVideoV2_4OrV5 || isVideoV10, 'v23-section-timeline-wrap--v6': isVideoV6OrV7 || isVideoV10 }"
                    :ref="(isVideoV2_4OrV5 || isVideoV10) ? (el => setSectionLineWrapRef(section.id, el)) : undefined"
                  >
                    <div v-if="isVideoV2_4OrV5 || isVideoV10" class="v23-section-timeline-wrap__line" aria-hidden="true" />
                  <component
                    :is="useAccordionChapters || isVideoV10 ? 'button' : 'div'"
                    :type="useAccordionChapters || isVideoV10 ? 'button' : undefined"
                    class="chapter-v2 chapter-v2--header"
                    :class="[
                      { 'chapter-v2--sticky-title-v23': isVideoV10 || (isVideoV2_3OrV24 && isSectionOpen(section.id)), 'chapter-v2--no-accordion': !(useAccordionChapters || isVideoV10), 'chapter-v2--v10-selected': isVideoV10 && v10SelectedChapterId === section.id, 'chapter-v2--v4-timeline': isVideoV2_4OrV5 || isVideoV10, 'chapter-v2--v6-timeline-right': isVideoV6OrV7 || isVideoV10 },
                    ]"
                    data-name="Chapter V2"
                    :aria-expanded="(useAccordionChapters || isVideoV10) ? isSectionOpen(section.id) : undefined"
                    @click="(useAccordionChapters || isVideoV10) ? toggleSection(section.id) : undefined"
                  ...
                  >
                    ...
                    <!-- V6/V7/V8 Learn tab: one video per chapter, scrollable (no sticky) -->
                    <section
                      v-if="isVideoV6OrV7 && courseTabsActive === 'content' && showVideoSectionVisible && section.videoAvailable && isSectionOpen(section.id)"
                      class="video-section video-section--inline video-section--scrollable"
                      :data-name="`VideoSection-${section.id}`"
                ...
                >
                  <component
                    ...
                    :class="{
                      ...
                      'chapter-v2--sticky-title-v23': isVideoV10 || (isVideoV2_3OrV24 && isSectionOpen(section.id)),
...
/* Viewport preset bar – responsiveness testing */
.viewport-preset-bar {
  flex-shrink: 0;
  display: flex;
  align-items: center;
...
.viewport-preset-bar__group {
  display: flex;
  align-items: center;
  gap: 6px;
}
.viewport-preset-bar__separator {
  width: 1px;
  height: 16px;
  background: rgba(255, 255, 255, 0.2);
  margin: 0 8px;
}
.viewport-preset-bar__btn {
  padding: 4px 10px;
  border-radius: 4px;
...
.viewport-preset-bar__btn:hover {
  background: rgba(255, 255, 255, 0.08);
}
.viewport-preset-bar__btn--active {
  background: rgba(93, 66, 143, 1);
  border: none;
  border-width: 0;
...
.viewport-preset-bar__btn--active:hover {
  background: linear-gradient(rgba(255, 255, 255, 0.05), rgba(255, 255, 255, 0.05)), rgba(93, 66, 143, 1);
}
...
/* Viewport preset: narrow – board same size, panel 300px */
...
.chapter-v2__timeline-col {
  flex-shrink: 0;
  width: var(--timeline-col-width, 24px);
  display: flex;
  align-items: center;
  justify-content: center;
  pointer-events: none;
...
.chapter-line-card .opening-course-card__inner {
  min-width: 0;
  flex-shrink: 1;
}
...
/* V9 only: course card always visible – sticky below tabs (legacy; V9 now uses fixed stack) */
/* V9 layout: fixed stack (header + tabs + course card) + scroll-only content. See .courses-content--v9-layout below. */
.courses-content--v9 .course-card-frame--v9-sticky {
  position: sticky;
  top: var(--tabs-h, 48px);
  z-index: 10;
  background-color: rgba(33, 31, 28, 1);
...
/* V9: one scroll container (content list only). Panel = flex column; stack = fixed height; scroll = flex 1. */
.courses-content.courses-content--v9 {
  display: flex;
  flex-direction: column;
  overflow: hidden;
  padding: 0;
}
.courses-content--v9-stack {
  flex-shrink: 0;
  position: relative;
  overflow: hidden;
  background-color: rgba(33, 31, 28, 1);
}
.courses-content--v9-stack__header {
  width: 100%;
  height: 0;
  min-height: 0;
  flex-shrink: 0;
...
/* V10: scroll div has BOTH .courses-content--v10 and .courses-content--v9-scroll (same element); chapter sticky top = 0 */
.courses-content--v10.courses-content--v9-scroll .chapter-v2--sticky-title-v23.chapter-v2--v10-selected,
.courses-content--v10.courses-content--v9-scroll .chapter-v2--sticky-title-v23.chapter-v2--no-accordion.chapter-v2--v4-timeline,
.courses-content--v10.courses-content--v9-scroll .chapter-v2--sticky-title-v23.chapter-v2--no-accordion.chapter-v2--v6-timeline-right {
  top: 0 !important;
  /* Avoid contain/will-change from other rules breaking sticky */
...
/* V7 content: height 80px (matches cover), title/author at top, progress row at bottom (baseline-aligned with cover) */
...
/* Learn tab: 1px bottom margin, progress row aligns with cover baseline */
.course-card-frame--with-completion .opening-course-card__content--v7-completion .course-card-completion-row {
  margin-bottom: 1px;
}
.course-card-completion-row .course-card-completion {
  flex: 0 0 auto;
  width: fit-content;
  min-width: 0;
  margin-top: 0;
...
/* V7 Practice tab only: same 80px height, space-between so mastery row sits at bottom (cover baseline) */
...
.course-card-mastery-group .course-card-av-level {
  justify-content: center;
  align-items: center;
}
...
.course-card-mastery-group .extra-data-label {
  margin-bottom: 0;
  padding-top: 3px;
  font-size: 12px;
}
.course-card-mastery-group .extra-data-practice-in .extra-data-time-chip-label {
  font-size: 12px;
}
.course-card--author {
  margin: 0;
...
.course-card-completion--in-mastery-row {
  margin-top: 0;
  width: fit-content;
  flex-basis: auto;
  align-items: center;
...
/* Narrow/mobile: match Mastery label (same size, style, color as .course-card-av-level .extra-data-label) */
.course-card-completion__label--compact {
  font-weight: 600;
  text-transform: none;
...
/* V6/V7/V8 Learn tab: per-chapter video scrolls with content (no sticky) */
...
/* V10: open selected collapsed chapter with no visible jump – suppress expand transition, set scroll, then re-enable */
.courses-content--v10.v10-expand-no-transition .v23-expandable,
.courses-content--v10.v10-expand-no-transition .v23-expandable--open {
...
/* Stats tab button: 48px height (override DS default 56px); system font, 5px gap */
.course-tabs-ds :deep(#tab-stats.cc-tab-item-component),
.course-tabs-ds :deep(.cc-tab-item-component#tab-stats) {
  height: 48px;
  font-family: system-ui, -apple-system, BlinkMacSystemFont, sans-serif;
  gap: 5px;
}
/* Tab labels: DS text-medium-bold (14px / 16px, weight 600); system font */
.course-tabs-ds :deep(.cc-tab-item-label) {
  font-size: 14px;
  line-height: 16px;
  font-weight: 600;
  font-family: system-ui, -apple-system, BlinkMacSystemFont, sans-serif;
}
/* Practice tab badge: 12px, system font, 30px radius (pill), aqua like L1/L2 chips */
.course-tabs-ds :deep(.cc-tab-item-component#tab-stats .cc-tab-item-badge) {
  font-size: 12px;
  line-height: 16px;
  font-weight: 600;
  font-family: system-ui, -apple-system, BlinkMacSystemFont, sans-serif;
  ...
  padding-top: 1px;
  padding-bottom: 1px;
  color: var(--color-aqua-300, #26C2A3) !important;
  background-color: rgba(38, 194, 163, 0.2) !important;
}
.course-tab-panel {
  margin-top: 0;
}
.course-tab-panel--video-hidden-v10 {
  min-height: 0;
  height: 0;
  overflow: hidden;
  padding: 0;
...
.course-tab-panel--hide-flash {
  opacity: 0;
  pointer-events: none;
}
/* V7/V8: stats drawer – overlay over TOC + drawer on top */
.stats-drawer-wrap {
  position: relative;
}
/* ToC wrap: overlay covers only this (not course card); needs position relative and flex to fill below card */
.stats-overlay-toc-wrap {
  position: relative;
  flex: 1;
  min-height: 0;
...
.stats-drawer-overlay {
  position: absolute;
  inset: 0;
  bottom: calc(-1 * (var(--space-16, 16px) + 12px));
  z-index: 10;
  background: rgba(0, 0, 0, 0.5);
  cursor: pointer;
}
.stats-drawer-overlay--fixed {
  position: fixed;
  inset: auto;
  bottom: auto;
}
...
.stats-drawer__title .course-progress-title {
  font-weight: 600;
  font-size: 14px;
}
/* Stats Header: hidden for later use; remove --hidden to show (must override .course-progress-header) */
.stats-drawer .stats-drawer__title.stats-drawer__title--hidden {
  display: none;
}
.stats-drawer__header {
  margin-bottom: 8px;
}
/* Stats icon lighter when drawer is open (5% white overlay on icon) */
.course-card-stats-icon.stats-icon--active {
  background: rgba(255, 255, 255, 0.05);
}
...
.course-tab-panel--stats .practice-empty-state {
  display: flex;
  flex-direction: column;
  align-items: center;
  gap: 8px;
...
.course-tab-panel--stats .practice-empty-state__image {
  width: 100px;
  height: 100px;
  object-fit: contain;
}
.course-tab-panel--stats .practice-empty-state__heading {
  margin: 0;
  font-size: 14px;
  font-weight: 400;
  line-height: 1.3;
...
.course-tab-panel--stats .practice-empty-state__description {
  margin: 0 40px;
  font-family: system-ui, -apple-system, BlinkMacSystemFont, sans-serif;
  font-size: 12px;
  line-height: 1.4;
...
/* Desktop S and mobile: no horizontal margins on description */
.app.app--viewport-narrow .course-tab-panel--stats .practice-empty-state__description,
.app.app--viewport-mobile .course-tab-panel--stats .practice-empty-state__description {
  margin-left: 0;
  margin-right: 0;
}

/* V7 Practice tab: compact mastery row – 192×12 segmented bar (same as course progress size) + Mastery on the right */
...
/* Practice All – Primary Large (DS) + badge for V6/V7; V8 uses aqua variant below */
...
/* V8 only: aqua Practice button (DS colors, same pattern as green primary) */
.courses-content--v8 .v6-btn-practice-all {
  background-color: var(--color-aqua-400, #109888) !important;
  color: #fff !important;
}
.courses-content--v8 .v6-btn-practice-all:hover {
  background-color: var(--color-aqua-500, #10777C) !important;
  color: #fff !important;
}
.courses-content--v8 .v6-btn-practice-all:active {
  background-color: var(--color-aqua-600, #09505F) !important;
  color: #fff !important;
}
/* Aqua Practice button: badge uses light bg + dark aqua number (V8 only) */
.courses-content--v8 .v6-btn-practice-all .v6-btn-practice-all-badge {
  background: rgba(255, 255, 255, 0.95);
  box-shadow: 0 1px 0 rgba(0, 0, 0, 0.15);
}
.courses-content--v8 .v6-btn-practice-all .v6-btn-practice-all-badge-text {
  color: var(--color-aqua-600, #09505F);
}

/* V9 only: same aqua Practice button as V8 (do not change V8 rules above) */
.courses-content--v9 .v6-btn-practice-all {
  background-color: var(--color-aqua-400, #109888) !important;
  color: #fff !important;
}
.courses-content--v9 .v6-btn-practice-all:hover {
  background-color: var(--color-aqua-500, #10777C) !important;
  color: #fff !important;
}
.courses-content--v9 .v6-btn-practice-all:active {
  background-color: var(--color-aqua-600, #09505F) !important;
  color: #fff !important;
}
.courses-content--v9 .v6-btn-practice-all .v6-btn-practice-all-badge {
  background: rgba(255, 255, 255, 0.95);
  box-shadow: 0 1px 0 rgba(0, 0, 0, 0.15);
}
.courses-content--v9 .v6-btn-practice-all .v6-btn-practice-all-badge-text {
  color: var(--color-aqua-600, #09505F);
}
...
/* V10: specific styles */
/* Outer panel must not scroll so sticky uses the inner scroll div (v9ScrollRef) as scroll container.
   Scroll container must start directly under the fixed stack (no gap). */
.panel-content.courses-content--v10 {
  overflow: hidden;
  display: flex;
  flex-direction: column;
  min-height: 0;
...
/* V10: layout-driven stack – no synthetic height; tabs container clips by --tabs-visible, card in flow */
.courses-content--v9-stack--v10-layout {
  flex-shrink: 0;
  display: flex;
  flex-direction: column;
  background-color: rgba(33, 31, 28, 1);
...
.courses-content--v9-tabs-visible-wrap {
  flex-shrink: 0;
  overflow: hidden;
}
.course-tabs-wrap--v10-in-flow {
  position: relative;
  transform: translateY(var(--tabs-y, 0));
  min-height: var(--tabs-h, 48px);
  background-color: rgba(33, 31, 28, 1);
  border-bottom: 1px solid rgba(255, 255, 255, 0.1);
  z-index: 15;
}
.courses-content--v9-course-card-wrap--v10-in-flow {
  position: static;
  flex-shrink: 0;
  background-color: rgba(33, 31, 28, 1);
}
.panel-content.courses-content--v10 > .courses-content--v10.courses-content--v9-scroll {
  margin-top: 0;
}
/* V10: ensure the scroll div is the only scroll container and gets a real height; no top gap */
.courses-content--v10.courses-content--v9-scroll {
  flex: 1 1 0;
  min-height: 0;
  overflow-y: auto;
  overflow-x: hidden;
...
/* V10: scroll content must start flush with top – no extra spacer above first chapter */
.courses-content--v10.courses-content--v9-scroll .stats-overlay-toc-wrap {
  padding-top: 0;
  margin-top: 0;
}
.courses-content--v10.courses-content--v9-scroll .stats-drawer-wrap {
  padding-top: 0;
  margin-top: 0;
}
/* V10 Learn: first Content panel (video) must stay collapsed so sections list is at top of scroll – same as Practice. If this panel had height: auto it would take space on reflow when tabs reveal on scroll up and create the gap. */
.courses-content--v10.courses-content--v9-scroll .course-tab-panel.course-tab-panel--video-hidden-v10 {
  height: 0;
  min-height: 0;
  overflow: hidden;
  padding-top: 0;
...
/* V10: all tab panels (Learn + Practice) flush to top so sticky chapter sits under course card in both tabs */
.courses-content--v10.courses-content--v9-scroll .course-tab-panel {
  padding-top: 0;
  margin-top: 0;
}
/* V10: no chapter-row-sized gap – sticky is the chapter row itself (.chapter-v2), not a wrapper; no placeholder/spacer */
.courses-content--v10.courses-content--v9-scroll .sections-list {
  padding-top: 0;
  margin-top: 0;
}
.courses-content--v10.courses-content--v9-scroll .section-item:first-child,
.courses-content--v10.courses-content--v9-scroll .section-item:first-child .v23-section-sticky-wrap,
.courses-content--v10.courses-content--v9-scroll .section-item:first-child .v23-section-timeline-wrap {
  margin-top: 0;
  padding-top: 0;
}
/* Ensure no extra min-height above first chapter (only the chapter row has min-height: 56px) */
.courses-content--v10.courses-content--v9-scroll .section-item:first-child .v23-section-sticky-wrap,
.courses-content--v10.courses-content--v9-scroll .section-item:first-child .v23-section-timeline-wrap {
  min-height: 0;
}
/* V10: first sticky chapter must sit flush under course card – no offset; sticky top is 0 relative to scroll container */
.courses-content--v10.courses-content--v9-scroll .section-item:first-child .chapter-v2--sticky-title-v23,
.courses-content--v10.courses-content--v9-scroll .section-item:first-child .chapter-v2--v10-selected {
  top: 0 !important;
